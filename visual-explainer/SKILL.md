...
description: |
  ...
  table, flowchart, dashboard, timeline, slide deck, or any visual explanation
  of technical concepts. Also use proactively when you are about to render a
  complex ASCII table (4+ rows or 3+ columns) — present it as a styled HTML
  page instead. Do NOT use for plain text responses or code-only output.
license: MIT
...
- Paper/ink (warm cream `#faf7f5` background, cornflower/teal accents, informal feel)
...
- Cornflower + teal (`#6495ED`, `#3491A3`) — clean, modern (default — use this when in doubt)
...
**Required:** Build palettes from the reference templates (cornflower/teal, rose/cranberry, amber/emerald, deep blue/gold) or derive from real IDE themes (Dracula, Nord, Solarized, Gruvbox, Catppuccin). Accents should feel intentional, not default.
...
### Keeping Up
- When new Mermaid diagram types are released, add them to the Diagram Types section
- When new font pairings prove effective, add them to references/libraries.md
- When new anti-patterns are identified, add them to the Anti-Patterns section
- When Chart.js or anime.js release major versions, update CDN URLs in references

### Known Limitations
- All examples assume macOS (`open` command); Linux uses `xdg-open` (documented)
- surf-cli image generation is optional and may not be installed
- Mermaid's `stateDiagram-v2` has label parsing limitations (documented inline)
- Very large diagrams (30+ nodes) may need manual layout guidance beyond what Mermaid provides
- No Windows support documented

### Learning from Use
- If a specific aesthetic consistently produces slop despite following the rules, add it to the forbidden list
- If a font pairing looks great, add it to the curated list in references/libraries.md
- If a new diagram type is frequently requested but not covered, add a section to Diagram Types
- If Mermaid rendering fails in specific browsers, document the workaround

### Testing Changes
After modifying this skill, test against the eval set in `evals/evals.json` to verify anti-slop enforcement, proactive table rendering, and Mermaid theming still work correctly.
